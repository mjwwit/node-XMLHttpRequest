...
function XMLHttpRequest(opts) {
  ...
  var createResponse = function(response, customContentType) {
    ...
    switch (self.responseType) {
      ...
      case "text":
        try {
          // Use TextDecoder for more supported charset encodings
          self.responseText = new TextDecoder(customEncoding || parseContentType(String(customContentType)).charset).decode(response);
        }
        catch (e) {
  ...
  this.send = function(data) {
    ...
    if (settings.async) {
      ...
      try {
        ...
        handleError(new Error("Synchronous operation aborted: Failed to perfoem read/write operations in operating system temporary directory."));
      }
