...
function XMLHttpRequest(opts) {
  ...
  var default_options = {
    ...
    textDecoder: function (buf, enc) {
      if ("function" === typeof TextDecoder) return new TextDecoder(enc).decode(buf);
  ...
  for (var i of ["xmlParser", "textDecoder"]) {
    if (opts[i] != null && typeof opts[i] !== "function") {
  ...
  var createResponse = function(response, customContentType) {
    ...
    switch (self.responseType) {
      ...
      default:
        try {
          self.responseText = opts.textDecoder.call(opts, response, customEncoding || parseContentType(String(customContentType)).charset);
        }
        catch (e) {
          // fall back to utf8 ONLY if custom encoding is present
          if (customEncoding) self.responseText = response.toString('utf8');
          else self.responseText = "";
        }
        self.response = self.responseText;
        try { self.responseXML = opts.xmlParser.call(opts, self.responseText); }
        catch (e) { self.responseXML = null; }
    }
    ...
    if (settings.async) {
      ...
      if (self.responseText.match(/^NODE-XMLHTTPREQUEST-ERROR(-REDIRECT){0,1}:/)) {
        ...
        if (self.responseText.startsWith('NODE-XMLHTTPREQUEST-ERROR-REDIRECT')) {
          handleError(new Error(self.responseText.replace(/^NODE-XMLHTTPREQUEST-ERROR-REDIRECT: /, "")));
        }
        else {
        ...
        self.response = fs.readFileSync(contentFile + ".bin");
