...
function XMLHttpRequest(opts) {
  ...
  if (max_redirects == null || isNaN(max_redirects)) max_redirects = 20;
  ...
  this.send = function(data) {
    ...
    var ssl = false, local = false, datauri = false;
    var url;
    try {
      url = new URL(settings.url);
    }
    catch (e) {
      self.handleError(new Error("ERR_INVALID_URL"));
      return;
    }
    var host;
    ...
    if (local) {
      if (datauri) try {
        // Triming from original url string since 
        var data = url.href.substr(5, settings.url.length);
        ...
        var base64 = false;
        // check if header part has base64
        var data_headers = parts[0].split(";");
        
        // ignore first element since it's MIME type
        for (var i = 1; i < data_headers.length; ++i) {
          if (data_headers[i].toLowerCase() === "base64") {
            base64 = true;
            break;
          }
        }
        ...
        if (base64) {
          ...
          inputData = inputData.substr(0, inputData.length - padding.length);
          ...
          responseData = Buffer.from(inputData);
        }

        self.status = 200;
        self.responseURL = settings.url;
        self.createFileOrSyncResponse(responseData);
        setState(self.DONE);
        return;
      }
      catch (e) {
      ...
      if (!opts.allowFileSystemSources) {
    ...
    if (settings.async) {
      ...
      var responseHandler = function(resp) {
        ...
        if (
          ...
          if (redirect_count > max_redirects) {
            self.handleError(new Error("ERR_TOO_MANY_REDIRECTS"));
            return;
          }
          // Change URL to the redirect location
          var url;
          try {
            url = new URL(response.headers.location, settings.url);
            ...
            self.handleError(new Error("ERR_UNSAFE_REDIRECT"));
            return;
          }
      ...
      var createRequest = function (opt) {
        ...
        if (ssl) Object.assign(opt, sslOptions);
      ...
      var execString = "'use strict';"
        ...
        + "        fs.writeFileSync('" + contentFile + "', 'NODE-XMLHTTPREQUEST-ERROR-REDIRECT: ERR_TOO_MANY_REDIRECTS', 'utf8');"
        ...
        + "      }"
        + "      catch (e) {"
        + "        fs.writeFileSync('" + contentFile + "', 'NODE-XMLHTTPREQUEST-ERROR-REDIRECT: ERR_UNSAFE_REDIRECT', 'utf8');"
        + "        fs.unlinkSync('" + syncFile + "');"
        + "        return;"
        + "      };"
        + "      ssl = url.protocol === 'https:';"
        ...
        + "      if (!((ssl && port === 443) || port === 80)) options.headers['Host'] += ':' + port;"
        + "      makeRequest();"
        + "      return;"
        + "    }"
        + "    response.on('data', function(chunk) {"
